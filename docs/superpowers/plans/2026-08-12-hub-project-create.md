...
**Architecture:** Add one declarative hub-owned `hub-project-create` workflow. It reuses the existing root list, registry/card schema, validator, active-project record, and shared post-confirmation workflows. No new installer, registry format, service, or standalone project entry files are added.
...
- If the target exists, write nothing and offer `hub-project-register` or a different name.
...
- [ ] Add a failing structural contract for `hub-template/ai/skills/hub-project-create/SKILL.md`.

It must require: routing mode; a confirmed allowed root; a new direct-child path; no project reads before confirmation; one explicit confirmation; collision rejection; `hub-project-register` for existing paths; the existing `scripts/check-hub-registry.sh`; all six named memory files; active-project update only after successful validation; hub-owned `hub-environment-check` and `hub-task-intake` after creation.
...
- [ ] Add `hub-project-create` to both existing required-hub-skill lists in `scripts/hub-smoke-test.sh` and `scripts/check-consistency.sh`.
...
**Files:**
- Create: `hub-template/ai/skills/hub-project-create/SKILL.md`

- [ ] Write a `hub-project-create` workflow with these steps:
  ...
  7. Only after successful validation, update `ai/active-project.md` with confirmed ID/path and invoke hub-owned `hub-environment-check` and `hub-task-intake`.
...
- [ ] Add one identical compact routing line to both hub entry files: a requested new project uses `hub-project-create`; an existing folder uses `hub-project-register`.
- [ ] Add to hub architecture: `hub-project-create` makes a new direct-child project with only six memory files, card, registry entry, and selection after one preview confirmation; existing folders stay with `hub-project-register`; no Git/code/dependencies/services/duplicate entries are created.
...
- [ ] Propose `hub-task-finish`; do not merge, push, or delete the worktree/branch without explicit confirmation.
