...
- [ ] Write Help in short Russian with sections: `Что можно сделать`, `Команды`, `Obsidian`, `Встречи`, `Подтверждение`, and `Если нужна помощь`.
- [ ] Rename the file and change its visible title to `Помощь`.
- [ ] Update `start-screen` wording and triggers: `помощь`, `что умеешь`, and `как работать`.
- [ ] Check that `rg -n 'getting-started/getting-started.md' README.md docs ai` has no live references.
...
- [ ] Add a short `Что нового` block for overview boards, review-only Obsidian sync, meeting notes, and safe direct opening of legacy hub projects.
- [ ] Replace the old start-screen wording with `Помощь` and link to `getting-started/help.md`.
- [ ] Rewrite the skills list as a short user command list, including project selection, tasks, Obsidian, meetings, project creation, updates, and Help.
- [ ] Keep installation instructions intact unless their Help link changed.
...
- [ ] Run `git diff --check`.
- [ ] Run `bash scripts/check-consistency.sh` and `bash scripts/smoke-test.sh`.
- [ ] Run `rg -n 'getting-started/getting-started.md' README.md docs ai` and require no result.
