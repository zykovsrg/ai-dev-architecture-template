...
- `evening-review` в `hub-workflows` читает календарь запрошенного дня,
  ...
  предположением: оно не доказывает выполнение, не расширяет область и не
  применяет изменение. Без выбранного файла ревью раздел «Сделано» строится из
  прошедших событий с явной пометкой. Архитектура повышена до 1.10.
...
- Мягкое правило краткости и обязательные секции формата вывода заменены
  одним правилом `Output` во входных файлах: по умолчанию не больше 5 строк
  и 80 слов, сначала ответ, затем при необходимости пояснение, простые слова
  вместо терминов, внутренние обозначения (`Mode:`, имена файлов памяти,
  названия воркфлоу) не показываются в ответе, один вопрос за сообщение.
  Экран подтверждения, который воркфлоу требует перед действием, — явное
  исключение и показывается всегда.
- То же правило добавлено в hub и в `hub-template/`, где вместо этого убран
  рабочий заголовок `Project:`/`Mode:`.
- `scripts/check-consistency.sh` теперь проверяет, что все шесть копий
  правила в репозитории совпадают дословно.
- Standalone-архитектура повышена до `7.5`, архитектура hub — до `1.9`.
