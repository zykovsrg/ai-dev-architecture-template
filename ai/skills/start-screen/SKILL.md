...
description: |
  ...
  Activates when the user explicitly asks "помощь", "что умеешь", "как работать",
  "покажи стартовый экран", "с чего начать", "как работать с архитектурой", or similar.
...
Use this skill only after an explicit user request. Present it as `Помощь`.
`start-screen` is only the internal compatibility name. This is a read-only
orientation screen, not a workflow and not an environment audit.
