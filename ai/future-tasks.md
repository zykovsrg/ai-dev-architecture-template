...
### FT-20260826-001 — Workflows: план дня и ревью
...
Foundation и безопасная Obsidian-проекция завершены. Следующий этап дорожной
карты — рабочие workflows, которые собирают данные и формируют предложения,
но не создают вторую базу задач и ничего не записывают сами.

Proposed task:

Спроектировать и реализовать «План дня», вечернее короткое ревью и недельное
ревью: фокус, waiting/follow-up, риски и сроки, предложения по встречам и
Calendar. Любое изменение задачи, срока, заметки или события оформлять точным
proposal и выполнять только после отдельного подтверждения пользователя.

Acceptance criteria:

- Есть проверяемый workflow для плана дня, вечернего и недельного ревью.
- Каждый workflow показывает предложения, а не выполняет запись сам.
- Есть тесты proposal-flow и защиты от автоматической записи.
- Существующие канонические `ai/`-файлы остаются единственным источником правды.

Promotion notes:

Перед реализацией создать текущую задачу через task-intake, подготовить план и
дождаться подтверждения. Не устанавливать Calendar MCP в рамках этой задачи.
...
### FT-20260826-003 — Пилот proposal-only session audit
...
Аудит сессий может находить улучшения, но чаты содержат чувствительный контекст.
Поэтому сначала разрешены только безопасные metadata, а чтение каждого
транскрипта требует выбора точных ID и отдельного подтверждения.

Proposed task:

Провести малый пилот session audit на выбранных пользователем сессиях. Выдавать
только evidence-backed предложения: future task, architecture proposal или
ничего. Не изменять память, код или настройки автоматически.

Acceptance criteria:

- До подтверждения точных ID видны только metadata.
- Пилот ограничен выбранными пользователем сессиями.
- Каждая находка содержит доказательство и является предложением.
- Без подтверждения нет записи в задачи, решения или настройки.

Promotion notes:

Начинать после Calendar-пилота и только после выбора пользователем точных
сессий для чтения.
