...
### FT-20260905-001 — Автоматическая сортировка входящих материалов в базе знаний

Status: idea

Priority: medium

Source: диалог 2026-09-05

Created: 2026-09-05

Context:

Материалы (заметки, изображения, презентации, файлы) сейчас раскладываются в Obsidian вручную. Это тормозит захват и приводит к накоплению неразобранного.

Proposed task:

Агент принимает произвольный входящий материал, определяет его тему и сущность и сам кладёт файл в подходящую папку хранилища Obsidian; если подходящей папки нет — предлагает создать.

Пример: отправлена презентация Ильяхова → агент распознаёт, что она относится к Ильяхову, находит его существующую папку и кладёт файл туда.

Promotion notes:

Решить, нужна ли подтверждающая проверка перед записью и что делать при неуверенном определении места.

### FT-20260830-001 — Предохранитель активного проекта при создании нового

Status: idea

Priority: high

Source: реструктуризация промо-страниц, сессия 2026-08-30

Created: 2026-08-30

Context:

При создании нового проекта `hub-project-create` автоматически меняет
`ai/active-project.md`. Если этот файл уже содержит незакоммиченное изменение
другого проекта, действие может перезаписать выбор пользователя.

Proposed task:

Добавить в `hub-project-create` проверку: при изменённом `ai/active-project.md`
не менять его автоматически. Показать выбор — сохранить текущий активный
проект или явно переключиться на новый. Создание и регистрация нового проекта
должны быть доступны в обоих вариантах.

Acceptance criteria:

- Создание проекта не перезаписывает незакоммиченное изменение
  `ai/active-project.md` без отдельного явного подтверждения.
- В предпросмотре ясно указан текущий активный проект и выбранный вариант.
- Реестр проходит `bash scripts/check-hub-registry.sh`.
- Есть тест или воспроизводимая проверка обоих вариантов.

Promotion notes:

Перед началом проверить, не покрывается ли ситуация уже другой процедурой
создания или переключения проекта.
...
### FT-20260901-001 — Ручной тест границ all-day по настоящему календарю

Status: ready

Priority: medium

Source: сессия 2026-09-01, баг с лишним днём у all-day события

Created: 2026-09-01

Context:

Семантика границ all-day целиком живёт в Swift-мосте: EventKit хранит конец
включительно (последний миг закрывающего дня), политика говорит на языке iCal,
где конец исключительный. Мост конвертирует в обе стороны. Ни один
автоматический тест этот код не выполняет: питоновские тесты работают со
stub-процессом, а Swift в `apple-calendar-bridge-test.sh` только компилируется.
Именно поэтому баг с лишними сутками нашёлся только на живом календаре.

Proposed task:

Отдельный ручной сценарий (не в общем наборе, запуск руками, нужен выданный
доступ и отдельный тестовый календарь): create → read → delete события
all-day. Проверить, что прочитанные границы равны переданным, что событие
занимает ровно одни сутки и попадает в верхний блок Apple Calendar. Отдельно
прогнать случай на переходе летнего времени.

Acceptance criteria:

- Сценарий описан пошагово и лежит рядом со скриптами calendar-policy.
- Он никогда не запускается из `apple-calendar-policy-test.sh`.
- Тестовый календарь и уборка за собой описаны явно.

Promotion notes:

Требует подтверждения пользователя на запись в календарь и выбранного
тестового календаря в allowlist.
